...
#读取文件
dataset= r'Formation energy After.xlsx'
...
#读取原数据集的特征和目标值
X,tx,y,ty=train_test_split(data.values[:,:-1], data.values[:,-1], test_size=0.2,random_state=1)
X = data.values[:,:-1]
y = data.values[:,-1]
...
#选取4种分类算法
# clf = RandomForestRegressor(n_estimators=100,random_state=80, min_samples_split=2)
...
# plt.rcParams['ytick.direction'] = 'in'
# plt.plot(y,y,label='Real Data')
# plt.scatter(y,py,label='Predict',c='r')
# ax=plt.gca()
# ax.spines['bottom'].set_linewidth(2);###设置底部坐标轴的粗细
# ax.spines['left'].set_linewidth(2);####设置左边坐标轴的粗细
# ax.spines['right'].set_linewidth(2);###设置右边坐标轴的粗细
